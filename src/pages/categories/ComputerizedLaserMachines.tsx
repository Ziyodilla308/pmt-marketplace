import {pmtProducts} from "@/data/pmtProducts.ts";
import CategoryFilters from "@/components/elements/CategoryFilters.tsx";
import {useCategoryFilter} from "@/hooks/useCategoryFilter.ts";
import ProductsGrid from "@/components/elements/Products.Grid.tsx";
import {useTranslation} from "react-i18next";

const ComputerizedLaserMachines = () => {
    const {t} = useTranslation();
    const {
        filteredProducts,
        setSelectedType
    } = useCategoryFilter(pmtProducts, "computerized-laser-machines");

    return (
        <div className="min-h-screen bg-background py-8">
            <div className="container mx-auto px-4">
                <div className="mb-8">
                    <h1 className="text-4xl font-bold mb-4">{t("computerizedLaserMachines")}</h1>
                    <p className="text-muted-foreground">
                        {t("professionalComputerizedLaserMachinesText")}
                    </p>
                </div>

                <div className="flex flex-col md:flex-row gap-8">
                    <aside className="w-full md:w-64 space-y-6">
                        <CategoryFilters
                            category="computerized_laser_machines"
                            onFilterChange={(t) => setSelectedType(t)}
                        />
                    </aside>

                    <ProductsGrid products={filteredProducts}/>
                </div>
            </div>
        </div>
    );
};

export default ComputerizedLaserMachines;
